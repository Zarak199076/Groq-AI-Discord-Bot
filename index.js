...
// Expands a short/vague user prompt into a detailed, vivid description before
// it hits the image model. This is usually the single biggest quality lever —
// diffusion models respond much better to specific, richly-described prompts
// than to a few words. Falls back to the original prompt if the LLM call fails.
async function enhanceImagePrompt(prompt) {
    try {
        ...
        console.warn(`Prompt enhancement failed, using original prompt: ${err.message}`)
        return prompt
    }
}

// Generates an image, preferring Hugging Face's FLUX.1-dev for quality
// (schnell is tuned for speed over fidelity), and falling back to Pollinations
// if the HF call fails (missing token, rate limit, cold-start timeout, etc.)
// so image generation never goes fully silent.
async function generateImage(prompt) {
    const finalPrompt = await enhanceImagePrompt(prompt)
    ...
    if (process.env.HF_TOKEN) {
        try {
            ...
            if (!contentType.startsWith('image/')) throw new Error(`HF returned non-image content: ${contentType}`)
            return Buffer.from(await res.arrayBuffer())
        } catch (err) {
            console.warn(`HF image generation failed, falling back to Pollinations: ${err.message}`)
        }
    }

    const seed = Math.floor(Math.random() * 1_000_000)
    const genUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(finalPrompt)}?width=1024&height=1024&seed=${seed}&nologo=true`
    const res = await fetch(genUrl)
    if (!res.ok) throw new Error('Both HF and Pollinations image generation failed.')
    return Buffer.from(await res.arrayBuffer())
}
    ...
    try {
        switch (toolCall.function.name) {
            ...
            case 'generate_image': {
                ...
                try {
                    const buffer = await generateImage(args.prompt)
